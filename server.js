...
app.prepare().then(() => {
  ...
  const io = new IOServer(server, { path: '/api/socket_io', cors: { origin: allowed, methods: ['GET','POST'], credentials: true } });
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('create_issue', ({ roomId, title }, cb) => {
      const room = rooms.get(roomId);
      if (!room) {
        cb?.({ ok: false, error: 'Sala não encontrada' });
        return;
      ...
      const issue = { id: issueId, title: title || 'Issue', votes: new Map(), revealed: false };
    ...
    socket.on('cast_vote', ({ roomId, issueId, value }, cb) => {
      const room = rooms.get(roomId);
      if (!room) {
        cb?.({ ok: false });
        return;
    ...
    socket.on('reveal_votes', ({ roomId }, cb) => {
      const room = rooms.get(roomId);
      if (!room) {
        cb?.({ ok: false });
        return;
    ...
    socket.on('next_issue', ({ roomId }, cb) => {
      const room = rooms.get(roomId);
      if (!room) {
        cb?.({ ok: false });
        return;
    ...
    socket.on('get_room_state', ({ roomId }, cb) => {
      const room = rooms.get(roomId);
      if (!room) {
        cb?.({ ok: false });
        return;
